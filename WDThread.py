# August 14, 2015  -- set for bluemix 
# Assumptions:
...
usingTopic1 = "test-thread"
usingTopic2 = "wd-thread"     
...
def LoadDataBuffer1(Buffer, ID, HeaderBuffer, messageString, counter, END):
    ...
    elif counter % trigger != 0 and END == False:  # bundle size hasn't reacher 200 or whatever trigger size is
        Buffer.write(messageString)  # write message to bundle
        checkLastLine(Buffer, ID, HeaderBuffer, counter)
    else:    
        SubmitDataBuffer(Buffer, ID, HeaderBuffer, counter)    # works up to here
        Buffer.seek(0)
        Buffer.truncate(0)
...
def checkLastLine(Buffer, ID, HeaderBuffer, counter):
    Buffer.seek(0)
    ...
    if listLength > 1:
        ...
        if var2 + 6 < var1:               # the lines in the file shouldn't vary by more that 4 digits
            ...
            SubmitDataBuffer(Buffer, ID, HeaderBuffer, counter)
            Buffer.seek(0)
            Buffer.truncate(0)
...
def SubmitDataBuffer(Buffer, ID, Header, counter): 
    ...
    if dada < 100 :       # this should discard any buffer less that 100 rows as
        ...
        print ("dada was less that 100 so we are discarding this batch")
        Buffer.seek(0)
        Buffer.truncate(0)
